...
describe('interest stream', () => {
  it('sets Cache-Control: no-cache for SSE and hijacks the reply', async () => {
    ...
    const { registerInterestStream, countInterestStreams } = await import('../src/realtime/interestStream');
    const previousKeepAlive = config.sseKeepAliveMs;
    config.sseKeepAliveMs = 0;

    const before = countInterestStreams();
    const reply = makeReply();
    registerInterestStream(makeRequest(), reply as any);
    ...
    expect(countInterestStreams()).toBe(before + 1);

    reply.close();
    expect(countInterestStreams()).toBe(before);
  ...
  it('adds CORS headers for allowed origins', async () => {
    ...
    const reply = makeReply();
    registerInterestStream(makeRequest({ origin: 'https://localloop.urbnia.com' }), reply as any);
  ...
  it('drops a subscriber whose socket throws on write instead of propagating', async () => {
    const { config } = await import('../src/config');
    const { registerInterestStream, broadcastInterest, countInterestStreams } = await import('../src/realtime/interestStream');
    const previousKeepAlive = config.sseKeepAliveMs;
    config.sseKeepAliveMs = 0;
    ...
    registerInterestStream(makeRequest(), healthy as any);
    registerInterestStream(makeRequest(), dead as any);
    expect(countInterestStreams()).toBe(before + 2);
